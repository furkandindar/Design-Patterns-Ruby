...
2- MathPoxy.rb is takes the IP that we collected from 1 (it gets the changed version) and passes it to https://newton.now.sh/simplify/ . This API gets the numbers and returns the result of them.

3- FactProxy.rb is takes the result that we collected from 2 and passes it to http://numbersapi.com/ . This api gets the number and returns a fact about the number that we sent. The fact is the output of this project.
